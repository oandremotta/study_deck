...
/// Gemini implementation via Firebase Functions proxy.
///
/// This service calls a Firebase Function that securely stores the API key
/// in Secret Manager, avoiding exposure of the key in client code.
class GeminiProxyService extends AiGenerationService {
  ...
  GeminiProxyService({required this.functionUrl}) {
    debugPrint('Initializing GeminiProxyService with Firebase Function');
  }
  ...
  Future<List<GeneratedCard>> generateCards(AiGenerationRequest request) async {
    try {
      ...
      debugPrint(
          'GeminiProxy: Generating ${request.cardCount} cards via Firebase Function...');

      final text = await _callFunction(prompt);

      if (text.isEmpty) {
        throw Exception('Resposta vazia do servidor');
      }

      debugPrint('GeminiProxy response received, parsing...');
      return _parseCardsResponse(text);
    } catch (e) {
      debugPrint('GeminiProxy generation error: $e');
      rethrow;
    }
  }
  ...
  Future<GeneratedCard> refineCard({
    ...
    try {
      ...
      debugPrint('GeminiProxy refine error: $e');
      rethrow;
    }
  }
  ...
  Future<PedagogicalFields> generatePedagogicalFields({
    ...
    try {
      ...
      debugPrint('GeminiProxy: Generating pedagogical fields...');
      ...
      debugPrint('GeminiProxy pedagogical response received, parsing...');
      return _parsePedagogicalFieldsResponse(text, question);
    } catch (e) {
      debugPrint('GeminiProxy pedagogical generation error: $e');
      rethrow;
    }
  }
...
Retorne APENAS um JSON valido (sem markdown, sem explicacoes):
{
  ...
  Future<String> _callFunction(String prompt) async {
    debugPrint('Calling Firebase Function: $functionUrl');
    ...
    if (response.statusCode != 200) {
      ...
      try {
        ...
        final message = error['error'] ?? 'Unknown error';
        throw Exception('Erro na API: $message');
      } catch (e) {
